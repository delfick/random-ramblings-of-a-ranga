...
<p>
  I've gone through a journey with static typing, starting from thinking static
  types were the programming equivalent of unnecessary bureaucracy to thinking
  it's insane to write any code without having some kind of type checker!
</p>
...
<p>
  ...
  started to figure out how to use mypy and what it looks like to take my style
  of Python and formalise how everything fits together. From the start of 2023
  I've had a few sprints where I tried to push the limits of mypy and I'm gonna
  use this post to explain the approaches I've currently landed on.
</p>

<Note>
  This post is around 5000 words excluding code examples, the read time for this
  post may be around 40 minutes.
</Note>
...
<p>
  ...
  large period of that I really mean an unhealthy type of nonstop) and I learnt
  how to use the language without types. What I’ve found as I learn to adapt my
  style to including these type annotations is a lot of what I did to make code
  reliable at runtime was the same things the type system formalises.
</p>

<p>
  The <mark>typing.Protocol</mark> provided to us is a way to declare the shape of
  some object. For those familiar with Go, they're like an Interface in that language,
  but with some more capability.
</p>
...
<Note>
  ...
  concept and so by definition lack an implementation, which is why all methods
  on a protocol should be empty (so either a doc comment, or those three dots)
</Note>

<p>
  And like Go interfaces (and unlike java interfaces) I don’t need to explicitly
  declare when an object satisfies this interface.
</p>
...
<p>
  In all of these cases if I had something that takes in a <mark>Thing</mark> I can
  use an instance of those objects to satisfy that interface
</p>
...
<p>
  ...
  that can be called without passing anything in and a None value is returned. What
...
<Note>
  ...
  more ways to call this method rather than less. If it was the other way round then
  the implementation would have less functionality than the protocol and the type
...
<p>
  In this example there is a protocol that represents the shape for some object,
  a protocol that represents the ability to create that object from two
  integers, and a single implementation that satisfies both protocols.
</p>
...
<ul>
  <li>
    <p>
      ...
      object with an <mark>__init__</mark> method that throws away what it receives
    </p>
  </li>
  <li>
    <p>
      The <mark>numbers</mark> attribute on such an object will either be
      dynamic when it should be static, or the <mark>__init__</mark> method needs
      to perform some calculations to set the value for it.
    </p>
  </li>
  <li>
    <p>
      When writing stub implementations of <mark>Stuff</mark> for tests, I have to
      know and care about the objects use to construct this object in deployed environments
      that I otherwise may not care about in that test (i.e. unnecessary coupling
...
<p>
  ...
  (having a choice is about the opportunity to do something just as much as it's
  about the opportunity to not do that something)
</p>

<p>
  There isn’t a purpose built way to do this in mypy but there is a trick that I
  find most people have a negative knee jerk reaction to, followed by trying to
  suggest alternatives, and then finally landing on acceptance:
</p>
...
<Note>
  ...
  errors closer to where we are explicitly trying to implement a protocol. There
  is no loss of functionality by not doing this, but it does produce better
  notices from mypy when an implementation made specifically to satisfy a
  protocol no longer satisfies that protocol.
</Note>
...
<p>
  ...
  Effectively this means this code is never executed. It is only ever statically
  analyzed. This isn’t strictly necessary but as python is an interpreted
  language, it brings the neglible runtime risk to zero including the risk that
  modules external to this one start depending on those objects at runtime, and
  tells other humans these words are explicitly only for the type checker
</p>

<p>
  The naming scheme I use here makes it such that each variable is prefixed with
  an underscore and is the name of the variable we are checking as an acronym.
  The idea is that in python the same name cannot be given different types and
  so we want a different name for each object we are checking but we don’t
  actually care what that name is. I tend to find most of the time an acronym
  will be unique enough that extra letters aren’t required.
</p>

<p>
  That cast is possibly the strangest part. It does make perfect sense though, I
  promise!
</p>
...
<p>
  ...
  at static time mypy will believe the value here has that declared type. So in
  this case we have the <mark>None</mark> value but mypy believes us that this
  value actually is an instance of the type we passed into
  <mark>cast</mark>. As a general rule I’m not a fan of cast and I think it’s
  easy to use it to create code that isn’t type safe, but it’s perfect for this
  situation.
</p>
...
<p>
  ...
  <mark>MyFirstCallable</mark>, those requirements can effectively be ignored as
  they aren't relevant to what this check is for.
</p>
...
<Note>
  ...
  </strong> It does the same checks in this block that it does everywhere else, but
  we're taking a shortcut here because we don't also need to make the code do something
  useful at runtime.
</Note>
...
<p>
  However this gives really poor errors cause, whilst the following description
  isn't technically true, mypy effectively sees this:
</p>
...
<p>
  I find the patterns above to be useful when generics aren’t involved, but they
  become especially useful when generics are involved. Especially when mypy
  strict mode is used (which I recommend, it makes it a lot more obvious when
  mypy isn't complaining because it's comparing against <mark>Any</mark> types)
</p>
...
<p>
  ...
  On the surface it appears they do the same thing and can be used for the same purpose.
...
<p>
  ...
  relationship between a protocol and an abc class (and potentially vice verca).
  And more importantly they serve different audiences.
</p>
...
<Note>
  ...
  <ul>
    <li>
      <p>
        The ability to mark specific methods as abstract such that subclasses
        cannot be instantiated if they don't have an implementation for those
        methods
      </p>
    </li>
    <li>
      <p>
        The ability to register non inherited classes as a member of that abc,
        so that the abc can be used in an <mark>isinstance</mark> check against the
        abstract class
      </p>
    </li>
  </ul>
  ...
  <p>
    For the purposes of my argument, any standard non protocol class can be the
    base class for the implementation, but for brevity I will keep referring to
    any such class as an "abc" class.
  </p>
</Note>

<p>
  This is relevant to a conversation about protocols because sometimes you need
  to do checks at runtime to know what shape an object is. And it’s really
...
<p>
  ...
  <strong>It does not check that those attributes are the correct type</strong> (and
  fundamentally can’t do that kind of check at runtime, especially when generics
  are involved)
</p>
...
<p>
  The downside is the code that does this kind of check does need to have known
  base classes that satisfy the protocol but I’d argue the situations where
  we’ve dynamically created some object that can be anything should be limited
  and part of that contract is saying it should be implemented in terms of some
  specific class (Protocol classes aren't "real" in the same way other classes
  are).
</p>
...
<p>
  ...
  concrete runtime object (in this case a subclass of some specific non protocol
  class) before we can then represent it in terms of a static time concept.
</p>
...
<p>
  ...
  >
    import_string
  </a> on some string you found on django settings. The return of that could be literally
  anything. And so we need a way to assert the type of that value matches what is
  expected.
</p>

<p>
  The pattern that I have landed on around this is to set an expectation that
  what is found will be treated as a callable object and we care about the type
  of whatever we get from invoking that object.
</p>
...
<Note
  >I think it's an anti-pattern to instead check that we've imported a specific
  class type like the following for the same reasons that it's a good idea to
  split the shape of an object from how that object is created.
...
<p>
  If you have a situation where you need to pass something in to that callable,
  then we have the same problem where we need to first assert a value is some
  concrete type before we can do anything with any kind of type safety. The
  pattern I advocate for is to expect an intermediary:
</p>
...
<p>
  ...
  always be treated as a callable that receives no arguments and either returns the
  thing we want, or the ability to make the thing we want given some extra runtime
  information.
</p>
...
<Note>
  ...
  at for this mechanism) it will always return something at the top level of a module.
...
<p>
  ...
  certainly manageable, it is a bit less simple and straight forward. Personally
  I see that as an education problem, same as every programming concept in any
  language. Some concepts require more effort to take on than others, but it can
  be easy sometimes to mistake unfamiliarity as complexity.
</p>

<p>
  The main difficulty when it comes to generics in mypy is we don't have "higher
  kinded type vars", which prevents the following:
</p>
...
<ul>
  ...
  <li>
    <p>
      Disconnect what is needed for the API from what is needed to implement the
      API.
    </p>
  </li>
...
<p>
  This becomes a bit more obvious when you have strict mode turned on as you’ll
  find mypy complains about requiring the type var gets filled in. So for
  example in the example above the type var for the collection is implicitly
  represented as
</p>
...
<p>
  ...
  The problem becomes whatever we fill it in with will restrict that type var in
  a way that cannot be further specialised. And it’s inevitable to reach a point
  where we end up with some code that we want to extend such that the Item has
  some extra method on it and we start having to do some dodgy casts to depend
...
<Note
  ...
  <mark>object</mark>
  represents no functionality whereas <mark>Any</mark> represents no types, and
  as a general rule avoiding <mark>Any</mark> makes for code that ages less poorly</Note
>
...
<Note>
  ...
  attributes and methods on the API, it's inputs, and it's outputs don't change
...
<p>
  ...
  make the types carry through would be to not have the <mark>T_Collection</mark
  > type var at all:
</p>
...
<p>
  Essentially we want to make it so we are defining a stable API in terms of an
  unstable source of data/logic and then it becomes a case of spending the
...
<p>
  ...
  these terms relate to how the types used to implement a container affects how
  different implementations can be substituted.
</p>

<p>
  So say we have a container generic called <mark>Container</mark> and it takes
  a single TypeVar, and we have two classes <mark>A</mark>
...
<Note>
  ...
  other variables. And I never use "T" or "U" as doing a find in a document for
  a single capital "T" is an awful experience.
  <p>
...
<p>
  Mypy will guide you to using the correct form but essentially if the type var
  is only ever used as an input type then it must be contravariant. If the type
  var is only ever used as an output it must be covariant, otherwise it is
  invariant.
</p>
...
<p>
  ...
  <mark>add_item</mark> and so mypy will complain at us if we define that type var
  as covariant.
</p>

<p>
  This is why the following is a liskov violation (mypy doesn't explicitly call
  this one out as violating the liskov substitution principle, but that's what
  is happening here)
</p>
...
<p>
  ...
  implementation will ignore the extra methods and outputs are covariant because
  the caller expects the extra methods. And the game is all about figuring out
  what happens when we substitute in a different implementation.
</p>

<h2>Different data, same functionality</h2>

<p>
  Trying to design an API using protocols first becomes a lot easier when as the
  developer you recognise that what the protocol needs does not need to include
  everything the implementation needs.
</p>
...
<p>
  ...
  of the data/logic flow had the wrong responsibilities by being awkward to work
  with. The more I was able to make for good coupling and cohesion in the
  design, the more helpful the protocols became. This however is the part that’s
  hard to sell: the need for a decent level of patient design work. Other than
  patterns that discourage easy-to-avoid foot guns I personally don’t have good
  answers for how to carry out that design work (the way I think about API
  design is highly contextual and difficult for me to generalise).
</p>
...
<p>
  ...
  obvious, and I do have a pattern that resolves that at the cost of some minor
  extra boilerplate
</p>
...
<p>
  ...
  having too many layers" and create an intermediary object that is specific to
  what is being operated on. This is a bit of a subtle distinction in this
...
<p>
  ...
  to have a record method directly on the items themselves, but it's easy to
  imagine a scenario where there's a 1:n relationship between item and
  "recording" functionality and this pattern lets us separate the action of this
  "record" from what that actually means so that it's the caller that controls
  what that means rather than the orchestrator.
</p>
...
<Note
  >Note that in both these situations, we are able to represent the two sides of
  the design coin such that the implementation is generic and the usage is not
  <Python
    source={`
import dataclasses
from typing import TYPE_CHECKING, Protocol, TypeVar, cast
...
<p>
  ...
  works really well for generics. For example, in comparison to this incorrect implementation:
</p>
...
<p>
  ...
  we're returning matches what we've specified. And, even worse, that first one
  effectively returns a shrug emoji!
</p>
...
<p>
  ...
  to be associated with an input to be bound to something specific. Essentially
  like simple algebra, given "X + 3", substitute in a value for "X" to solve the
  equation.
</p>
...
<p>
  And then we can create our intermediary class used to create the final result
  in a way that's type safe:
</p>
...
<p>
  The other getter might be a little simpler depending on whether our base class
  is generic or not. Easiest case being we have a base class that isn't generic
  itself:
</p>
...
<p>
  When I think about API design, I think of it as the "two sides of the coin" as
  I've mentioned in passing above, where you have the API from the perspective
  of how it's used and the API from the perspective of how it's implemented. And
  that perspective is relative to what/who is using the code rather than
  relative to the code itself.
</p>
...
<p>
  ...
  their code when the implementation changes, and different implementations tend
  to think about the solution in different ways.
</p>

<p>
  This means it's possible, and good, to create different abc classes that all
  satisfy the same interface, but prioritise different aspects of how to achieve
  that interface. This means you may have an abc class that already has an
  implementation for the primary interface and requires subclasses to implement
  a different set of methods to fill in the blanks. Whereas a different abc for
  the same primary interface may do the same but for an entirely different set
  of methods, or indeed only care about the primary interface. In all cases,
  these extra methods are not relevant to the primary interface and the caller
  should not know about them, and the static type checker ain't gonna complain
...
<p>
  ...
  different, non overlapping needs from the same object. Perhaps one part of the
  codebase only requires an interface for writing data, whereas a different part
  of the codebase only requires reading data. This is especially obvious when
  you create stub implementations for tests and find yourself creating a method
  that raises <mark>NotImplementedError</mark>
...
<p>
  ...
  something like <mark>class P_Item</mark> and I'd rather keep the protocols as "clean"
  as possible to make them more natural to use
</p>
...
<p>
  This also means I do have one place that defines the API surface that my code
  wants to provide and work with
</p>
...
`}
  ...
  <p>
    Which will work as long as the files within that module don't do an import
    that accesses attributes on the module itself. So this is fine:
  </p>
...
<p>
  ...
  transformation after instantiation to be complete, then you actually have two
  objects. One object representing the first "wave" of data, and an object that
  represents the transformation of that data.
</p>

<p>
  A great example of this is in a mypy plugin I made for Django projects that
  requires runtime Django introspection to do what it needs to do. I start with
  information about where the project is and what is needed to start it up. I
  use this <mark>Project</mark> object to create a
  <mark>Loaded</mark>
...
<p>
  ...
  information that requires an operation be defined as optional values on the <mark
    >Project</mark
  >. By representing these different states as different objects I remove the
  need to do a bunch of
  <mark>is None</mark> checks all over the place to ensure that these optional values
  have values.
</p>
...
<Note>
  <p>
    Something to notice about this example is that on each protocol class, all
    the attributes are decorated with <mark>@property</mark> which means that
    they are read only. I'd love to instead have something like
...
<p>
  Hopefully this has been useful. It's a different mindset to think about Python
  from a static perspective. It's such a dynamic language and the best kind of
  python is able to lean into that without relying on hidden side effects. I
  really do believe that having static types can be used to add structure
  without removing flexibility, and tends to only very awkward when we're doing
  things we shouldn't be doing anyways!
</p>
