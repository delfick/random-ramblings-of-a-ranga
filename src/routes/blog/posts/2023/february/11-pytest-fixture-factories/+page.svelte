...
<p>
    These are then found by pytest by traversing the scope outwards from the
    test. So if a test is on a class, then pytest will look at
    methods/attributes on the class, then at the module level of the file the
    test is in, then the conftest.py file of each package beyond that
</p>

<p>
    This means it's possible to put a fixture in a conftest.py file and it'll be
    discovered by any test below that conftest.py. This is useful to a point,
    but after a large enough codebase it can become difficult to track where
    fixtures come from.
</p>

<p>
    A pattern that I'm starting to like is where I have a factory that spits out
    a fixture and then I create that fixture at the top of the file where I use
    it
</p>
...
<p>
    I wanted the other day to test that my Django template was getting href
    values from url arguments and displaying those. I also didn't want my test
    to have a specific root urlconf (it makes sense in context) and so I only
    cared that the correct url names/arguments were being used
</p>
...
<p>
    ...
    used, so that the dependency injection by name only retains enough meaning
...
<p>
    ...
    factory is on a class and the fixture gets us an instance of that class.
    That way we import one thing to make the fixture and type hint it
</p>
...
<p>
    The above is cool and all, but now the functionality of <code
        >ReverseResult</code
    > is consumed by the fact that it's a fixture and this makes it difficult to
    get to that functionality if we aren't using it as a fixture!
</p>
...
<p>
    For this to work we need to also make our class a context manager. In Python
    a context manager is essentially an object that is compatible with pythons
...
<p>
    ...
    tools that make contextmanagers easier to make and work with. The first
    we'll use is the contextmanager decorator:
</p>
...
<p>
    The second tool that can help us is the ExitStack. This is an object that
    lets us use the context manager without extra indentation
</p>
<p>
    <Python
...
<p>
    The last nice thing about this pattern is that factories are flexible, you
    can ask the factory to customize what it spits out!
</p>
...
<p>
    I really like this pattern. For the cost of some context manager tricks and
    a classmethod I get
</p>
<ul>
    <li>
        Names of fixtures don't need to be globally sensitive, which becomes
        more important the larger your collections of tests become
    </li>
